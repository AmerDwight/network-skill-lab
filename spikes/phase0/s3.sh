...
run_node() {
  docker run -d --name "$1" --hostname "$1" --privileged --cgroupns=private \
    --tmpfs /run --tmpfs /run/lock --tmpfs /tmp --mount type=volume,dst=/var/lib/rancher/k3s \
...
check "node k3s01 internal IP is eth1" sh -c "$(declare -f k); k get node k3s01 -o jsonpath='{.status.addresses[?(@.type==\"InternalIP\")].address}' | grep -q 10.0.8.10"
for _ in $(seq 1 240); do
  pend="$(k get pods -A --no-headers 2>/dev/null | awk '$4!="Running" && $4!="Completed"' | wc -l)"
...
check "all system pods Running/Completed" test "$(k get pods -A --no-headers | awk '$4!="Running" && $4!="Completed"' | wc -l)" -eq 0
...
check "pods running on distinct nodes" sh -c "$(declare -f k); k wait --for=condition=Ready pod/p1 pod/p2 --timeout=90s"
...
check "p1 pings p2 across nodes" sh -c "$(declare -f k); k exec p1 -- ping -c 2 -W 2 $p2ip"
check "p1 resolves kubernetes.default via coredns" sh -c "$(declare -f k); k exec p1 -- nslookup kubernetes.default"
check "vxlan device flannel.1 exists on agent" x $A sh -c "ip -d link show flannel.1 | grep -q vxlan"

section "offline proof"
check "no image pull attempted on server" sh -c "! docker exec $S journalctl -u k3s --no-pager | grep -qiE 'pulling image|failed to pull'"
check "no image pull attempted on agent"  sh -c "! docker exec $A journalctl -u k3s-agent --no-pager | grep -qiE 'pulling image|failed to pull'"
check_out "images imported on agent" x $A sh -c "crictl images -q | wc -l"
